# Truemeds Price Lock — Claude Rules

## What this project is

This project captures Truemeds Price Lock / Cost Absorption decisions across Pre-order, WMS, invoice, pricing, tax, payments, and fulfilment dependencies.

This is a product-discovery and decision workspace. Do not create a PRD unless the user explicitly asks for one.
...
8. `docs/context/ORDER_INVOICE_48188241_Paid.pdf` — real batch-level invoice, canonical for any invoice-layout/discount-line question
...
- Use `docs/context/project_truth.md` as current product truth.
- Use `docs/context/decision_log.md` for durable decisions and reversals.
- Use `docs/context/open_questions.md` for unresolved items.
- Use `docs/context/session_handoff.md` to resume work without chat history.
- Treat `reference/CAB/` as historical reference, not current truth.
- Never edit original files in `/Users/mac/Documents/CAB`.
- Mark facts, user decisions, proposals, and open questions separately.
- Challenge weak framing; do not silently convert assumptions into requirements.
- Keep Price Lock scope separate from coupons, rewards, credits, charges, tax, payment, refund, returns, substitution, and pack-size orchestration.
- Preserve batch-level and line-version traceability.
...
- Keep this project documentation-first until product truth and open questions are resolved.

---

## What requires explicit user confirmation before implementing

- Changing locked BSP definition.
- Changing same-pack scope boundary.
- Adding pack-size or substitution orchestration to Price Lock.
- Changing threshold ownership or fallback.
- Changing tax, refund, returns, or customer-benefit policy.
